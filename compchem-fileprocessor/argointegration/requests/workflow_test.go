...
func TestBuildWorkflow_IntegrationTest(t *testing.T) {
	...
	assert.Equal(
		t,
		"test.txt test1.txt",
		workflow.Spec.Arguments.Parameters[1].Value,
	) // Empty string as no file IDs were provided
	...
	assert.Equal(t, []string{}, tasks[0].Dependencies)
	...
	assert.Equal(t, []string{"read-files-12345-2"}, tasks[1].Dependencies)
	...
	assert.Equal(
		t,
		"{{tasks.count-words-12345-2.outputs.artifacts.output-files}}",
		tasks[2].Arguments.Artifacts[0].From,
	)

	// Check fourth task (write)
	assert.Equal(t, "write-files-12345-2", tasks[3].Name)
	assert.Equal(t, []string{"count-words-advanced-12345-2"}, tasks[3].Dependencies)
	...
	assert.Equal(
		t,
		"{{tasks.count-words-advanced-12345-2.outputs.artifacts.output-files}}",
		tasks[3].Arguments.Artifacts[0].From,
	)
